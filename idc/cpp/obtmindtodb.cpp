...
int main(int argc,char *argv[])
{
    ...
    if (argc != 5)
    {
        ...
        printf("Example:/project/tools/bin/procctl 10 /project/idc/bin/obtmindtodb /idcdata/surfdata "\
                  "\"idc/idcpwd@snorcl11g_124\" \"Simplified Chinese_China.AL32UTF8\" /log/idc/obtmindtodb.log\n\n");
...
bool _obtmindtodb(const char *pathname,const char *connstr,const char *charset)
{
    ...
    while (true)
    {
        ...
        while(true)
        {
            ...
            if (bisxml==true)
            {
                if (ifile.readline(strbuffer,"<endl/>")==false) // xml文件的行结束标志是<endl/>
                break;
            }
            else
            {
                if (ifile.readline(strbuffer)==false)   // csv文件没有行结束标志。
                break;
            }
