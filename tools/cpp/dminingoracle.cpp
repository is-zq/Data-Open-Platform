...
void _help()
{
    ...
    printf("       /project/tools/bin/procctl   30 /project/tools/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND.log "\
              ...
              "<maxcount>1000</maxcount><connstr1>scott/zq2003@snorcl11g_124</connstr1>\"\n\n");
    ...
    printf("starttime   程序运行的时间区间，例如02,13表示：如果程序启动时，只在02时和13时运行，其它时间不运行。"\
         "如果starttime为空，表示不限制，只要本程序启动，就会执行数据抽取任务，为了减少数据源数据库压力"\
...
bool instarttime()
{
    // 程序运行的时间，例如02,13表示：如果程序启动时，只在02时和13时运行，其它时间不运行。
    if (strlen(starg.starttime)!=0)
    {
        string strhh24=ltime1("hh24");  // 获取当前时间的小时，如果当前时间是2025-01-21 12:35:40，将得到12。
...
bool _dminingoracle()
{
    ...
    string strxmlfilename;  // 输出的xml文件名，例如：ZHOBTCODE_20250213162835_togxpt_1.xml
...
bool readincfield()
{
    ...
    if (strlen(starg.connstr1) != 0)
    {
        // 从数据库表中加载递增字段的最大值。
        connection conn1;
        if (conn1.connecttodb(starg.connstr1,starg.charset)!=0) //递增字段最大值存在共享平台的数据库，而不是各部门的数据源数据库
        {
...
bool writeincfield()
{
    ...
    else
    {
        ...
        // 把已抽取数据的最大id写入文件。 注：开发框架有bug，这里必须加\n换行
        ofile.writeline("%ld\n",imaxincvalue);
    }
