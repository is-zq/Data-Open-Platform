//增量同步程序：将远程数据库的数据增量同步到本地，同步的数据只能增加。
#include "_tools.h"
...
void _help(char *argv[])
{
    ...
    printf("把T_ZHOBTMIND1@db124表中全部的记录增量同步到T_ZHOBTMIND2中。\n");
    printf("Sample:/project/tools/bin/procctl 10 /project/tools/bin/syncinc /log/idc/syncinc_ZHOBTMIND2.log "\
              "\"<localconnstr>idc/idcpwd@snorcl11g_128</localconnstr><remoteconnstr>idc/idcpwd@snorcl11g_124</remoteconnstr>"\
              "<charset>Simplified Chinese_China.AL32UTF8</charset>"\
              "<remotetname>T_ZHOBTMIND1</remotetname><lnktname>T_ZHOBTMIND1@db124</lnktname>"\
    ...
    printf("把T_ZHOBTMIND1@db124表中满足\"and obtid like '57%%'\"的记录增量同步到T_ZHOBTMIND3中。\n");
...
bool _syncinc(bool &bcontinue)
{
    ...
    char remkeyidvalue[21];       // 从远程表查到的需要同步记录的rowid。
    ...
    stmtsel.bindout(1,remkeyidvalue,20);
    ...
    char keyidvalues[starg.maxcount][21];

    // 准备插入本地表数据的SQL语句，一次插入starg.maxcount条记录。
    // insert into T_ZHOBTMIND3(obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid)
    //                   select obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid from T_ZHOBTMIND1@db124
    ...
    for (int ii=0;ii<starg.maxcount;ii++)
    {
        stmtins.bindin(ii+1,keyidvalues[ii]);
    }

    memset(keyidvalues,0,sizeof(keyidvalues));
    ...
    while (true)
    {
        ...
        strcpy(keyidvalues[ccount],remkeyidvalue);
