...
// 判断当前时间是否在程序运行的时间区间内。
bool instarttime();
// 业务处理主函数。先select出符合条件的记录再分批insert+delete，避免大事务。
bool _migratetable();
...
bool _migratetable()
{
	...
	while (true) 
	{
        ...
        // b）如果数组中的记录数达到了starg.maxcount，执行一次迁移数据的SQL语句。
